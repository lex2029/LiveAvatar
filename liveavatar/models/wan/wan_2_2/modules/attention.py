...
def _sdpa_fallback(q, k, v, q_lens=None, k_lens=None, dropout_p=0.0, causal=False, dtype=torch.bfloat16):
    ...
    q_t = q.to(half_dtype).transpose(1, 2)
    k_t = k.to(half_dtype).transpose(1, 2)
    v_t = v.to(half_dtype).transpose(1, 2)
...
def attention(
    ...
    def cudnn_require():
        if os.getenv("LIVEAVATAR_DISABLE_CUDNN_ATTN", "true").lower() == "true":
            return False
        if window_size != (-1, -1):
            return False

...
class WanS2V:
    ...
    def __init__(
        ...
        self._noise_model_dir = noise_model_checkpoint_dir or checkpoint_dir
        ...
        logging.info(f"Creating WanModel from {self._noise_model_dir}")
        if not dit_fsdp:
            self.noise_model = CausalWanModel_S2V.from_pretrained(
                self._noise_model_dir,
                torch_dtype=self.param_dtype,
                device_map=self.device)
        else:
            self.noise_model = CausalWanModel_S2V.from_pretrained(
                self._noise_model_dir, torch_dtype=self.param_dtype)
